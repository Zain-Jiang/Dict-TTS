...
from tasks.tts.dataset_utils import BaseTTSDataset, FastSpeechWordDataset
...
class TTSBaseTask(BaseTask):
    ...
    def build_word_encoder(self, data_dir):
        word_list_file = os.path.join(data_dir, 'word_set.json')
        word_set = json.load(open(word_list_file))
    ...
    def plot_mel(self, batch_idx, spec_out, spec_gt=None, name=None, title=''):
        ...
        self.logger.add_figure(name, spec_to_figure(spec_out, vmin, vmax, title=title), self.global_step)
    ...
    def save_result(wav_out, mel, base_fn, gen_dir, str_phs=None, mel2ph=None, alignment=None):
        ...
        f0 = f0 / 10 * (f0 > 0)
        plt.plot(f0, c='red', linewidth=5, alpha=0.6)
        mel2ph = None
        plt.axis('off')
        plt.xticks([]) 
        plt.yticks([]) 
