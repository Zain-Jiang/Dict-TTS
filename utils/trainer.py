    ...
    def restore_weights(self, checkpoint):
        ...
        if len([k for k in checkpoint['state_dict'].keys() if '.' in k]) > 0:
            ...
            for k, v in checkpoint['state_dict'].items():
                # Init the parameters for spk_embedding for finetune
                if hparams.get('finetune_from_asr', False):
                    weight = torch.zeros(hparams['num_spk'], hparams['hidden_size'])
                    if 'spk_embed_proj.weight' in v:
                        if not (weight.shape[0] == v['spk_embed_proj.weight'].shape[0] and weight.shape[1] == v['spk_embed_proj.weight'].shape[1]):
                            # v['spk_embed_proj.weight'] = torch.nn.Parameter(torch.nn.init.xavier_normal_(weight))
                            if 'spk_embed_proj.weight' in v:
                                del v['spk_embed_proj.weight']
                            if 'spk_embed_proj.bias' in v:
                                del v['spk_embed_proj.bias']
