...
def collate_3d(values, pad_idx=0, left_pad=False, shift_right=False, max_len=None):
    """Convert a list of 3d tensors into a padded 4d tensor."""
    size_1 = max(v.size(0) for v in values) if max_len is None else max_len
    size_2 = max(v.size(1) for v in values) if max_len is None else max_len
    res = values[0].new(len(values), size_1, size_2, values[0].shape[2]).fill_(pad_idx)
    ...
    def copy_tensor(src, dst):
        ...
        if shift_right:
            ...
            dst.copy_(src)
    for i, v in enumerate(values):
        copy_tensor(v, res[i][size_1 - v.size(0):, size_2 - v.size(1)] if left_pad else res[i][:v.size(0), :v.size(1)])
    return res
