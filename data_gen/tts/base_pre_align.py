...
class BasePreAlign:
    ...
    def process_wav(idx, item_name, wav_fn, processed_dir, pre_align_args):
        if pre_align_args['sox_to_wav'] or pre_align_args['trim_sil'] or \
            ...
            subprocess.check_call(f'sox "{wav_fn}" -t wav -b 16 "{new_wav_fn}.wav"', shell=True)
    ...
    def process(self):
        ...
        if hparams['use_dict']:
            ...
            for l in word_level_dict:
                char = l[0]
                heteronym = pinyin(char, heteronym=True, style=Style.TONE3, strict=False)[0]
                for item in heteronym:
                    word_level_dict_.append(char + ' ' + item)
                    # shengmu = to_initials(item, strict=False)
                    # yunmu = to_finals_tone3(item, strict=False)
                    # if shengmu == yunmu:
                    #     word_level_dict_.append(char + ' ' + shengmu)
                    # else:
                    #     word_level_dict_.append(char + ' ' + shengmu + ' ' + yunmu)
            word_level_dict = word_level_dict_
