...
class TxtProcessor(BaseTxtProcessor):
    ...
    def pinyin_with_en(cls, txt, style):
        x = pinyin(txt, style, strict=False)
        x = [t[0] for t in x]
        x_ = []
        for t in x:
            if '$' not in t:
    ...
    def process(cls, txt, pre_align_args):
        ...
        txt = txt.replace("嗯", "恩") # pypin会把嗯的声母韵母识别为''，导致ph2word出现错位。
        ...
        seg_list = psg.lcut(txt)
        seg_list = TxtProcessor.sandhi_processor.pre_merge_for_modify(seg_list)
        seg_list = [word for word, pos in seg_list]
        ...
        for i in range(0, len(seg_list), 1):
            if seg_list[i] != '#' or (seg_list[i - 1] not in sil_phonemes and seg_list[i + 1] not in sil_phonemes):
                word_list.append(seg_list[i])
